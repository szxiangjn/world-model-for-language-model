    ...
    def execute(self, script: Script, state: EnvironmentState, info: ExecutionInfo):
        ...
        for node in state.select_nodes(current_obj):
            ...
            if self.check_walk(state, node, info) and node_room is not None:
                ...
                new_closes = _find_nodes_from(state, node, relations=[Relation.ON, Relation.INSIDE, Relation.CLOSE])
                if node.class_name not in ['home_office', 'bathroom', 'dining_room', 'bedroom']:
                    new_closes += _find_nodes_to(state, node, relations=[Relation.INSIDE])
                # if new_close is not None:
                for new_close in new_closes:
